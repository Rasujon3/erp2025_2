...
@section('content')
    <div class="container-fluid">
        @include('leads::leadStatus.partials.content_header')
            ...
            <div class="col-md-12" id="mainColumn">
                ...
                <div class="tab-content mt-3" id="myTabContent">
                    ...
                    <div class="tab-pane fade show active" id="grid" role="tabpanel">
                        @include('leads::leadStatus.partials.card_view')
                    </div>

                    <!-- List View Tab -->
                    <div class="tab-pane fade" id="list" role="tabpanel">
                        @include('leads::leadStatus.partials.list_view')
                    </div>
                </div>

                @include('leads::leadStatus.partials.modals')
            </div>
            {{-- below filter --}}
            @include('leads::leadStatus.partials.side_filter')
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadLeadStatusesGridView(loadMore = false, searchQuery = null, pageNumber = null) {
                ...
                $.ajax({
                    url: '{{ route('lead.status.grid-view') }}',
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        const editRoute = "{{ route('lead.status.edit', ':id') }}";
                        const viewRoute = "{{ route('lead.status.view', ':id') }}";
                        const data = response.data;
                        if (data.length) {
                            dataSize = data.length;
                            data.forEach(leadStatus => {
                                let colorData = [{ 'color': leadStatus.color }];
                                 ...
                                 <div class="col-md-3 col-lg-3  ">
                                    <div class="customer-card border rounded p-3">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <p class="mb-0 txtColor">Color</p>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <p class="mb-0 fw-bold">${leadStatus.name}</p>
                                            <p class="mb-0 fw-bold">${renderedHtml}</p>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <p class="mb-0 txtColor">Order</p>
                                            <p class="mb-0 txtColor">Leads</p>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <p class="mb-0 fw-bold">${leadStatus.order}</p>
                                            <p class="mb-0 fw-bold">'N/A'</p>
                                        </div>
                                        <div class="card-footer text-end mt-2">
                                            <a href="${editRoute.replace(':id', leadStatus.id)}" class="btn btn-primary btn-sm" title="Edit">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <button class="btn btn-danger btn-sm delete-currency" data-id="${leadStatus.id}" title="Delete">
            ...
            $(document).on('click', '.btnRefreshCard', function() {
                $('#tableSearch').val('');
                currentPage = 1;
                loadLeadStatusesGridView(false, null, 1);
            });
            ...
            $(document).on('click', '#confirmDelete', function() {
                ...
                const deleteCountryUrl = "{{ route('lead.status.destroy', ':id') }}".replace(':id', id);
                $.ajax({
                    ...
                    error: function(xhr) {
                        $('#deleteCardModal').modal('hide'); // Hide the modal
                        changeButton("#confirmDelete", false);
                        showToast('error', 'Failed to delete the Lead status. Please try again.');
            ...
            $('.card-container').on('scroll', function() {
                ...
                if (container.scrollTop() + container.innerHeight() >= container[0].scrollHeight - 50) {
                    ...
                    if (!isLoading && dataSize >= 1) {
                        ...
                        loadLeadStatusesGridView(true); // Load more data
                    }
                }
            });
            // Initial load
            ...
            searchInput.on('input', function() {
                ...
                debounceTimer = setTimeout(function() {
                    loadLeadStatusesGridView(false, searchQuery,
            ...
            function loadLeadStatusesDataTable() {
                ...
                let table = $('#currencies-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('lead.status.getAllLeadStatuses') }}",
                    dataSrc: '',
                    responsive: true,
                    columns: [
                        {
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                let colorData = [{ 'color': row.color }];
                                return prepareTemplateRender('#leadStatusColorBox', colorData);
                            }
                        },
                        {
                            data: 'order',
                        ...
                        // },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                const viewRoute = "{{ route('lead.status.view', ['leadStatus' => ':id']) }}";
            ...
            loadLeadStatusesGridView(); // Initial load
