...
@section('content')
    <div class="container-fluid">
        @include('admin::countries.partials.content_header')
            ...
            <div class="col-md-12" id="mainColumn">
                ...
                <div class="tab-content mt-3" id="myTabContent">
                    ...
                    <div class="tab-pane fade show active" id="grid" role="tabpanel">
                        @include('admin::countries.partials.card_view')
                    ...
                    <div class="tab-pane fade" id="list" role="tabpanel">
                        @include('admin::countries.partials.list_view')
                    </div>
                </div>

                @include('admin::countries.partials.modals')
            </div>
            {{-- below filter --}}
            @include('admin::countries.partials.side_filter')
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            function loadCountriesGridView(loadMore = false, searchQuery = null, pageNumber = null) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Update the summary values dynamically using the response data
                        $('#totalCountries').text(response.summary.totalCountries);
                        $('#totalDraft').text(response.summary.totalDraft);
                        $('#totalInactive').text(response.summary.totalInactive);
                        $('#totalActive').text(response.summary.totalActive);
                        $('#totalUpdated').text(response.summary.totalUpdated);
                        $('#totalDeleted').text(response.summary.totalDeleted);

                        const editRoute = "{{ route('countries.edit', ':id') }}";
                        const viewRoute = "{{ route('countries.view', ':id') }}";
                                 ...
                                 <div class="col-md-3 col-lg-3  ">
                                    <div class="customer-card border rounded p-3">
                                        <div class="d-flex justify-content-between">
                                            <p class="mb-0 txtColor">Code</p>
                                            <p class="mb-0 txtColor">Status</p>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <p class="mb-0 fw-bold">${currency.code}</p>
                                            <p class="mb-0 ">
                                                ${currency.is_active ? 'Active' : ''}
                                                ${currency.is_default ? 'Default' : ''}
                                                ${currency.draft ? 'Draft' : ''}
                                            </p>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between pt-2">
                                            ...
                                            <div class="flex-shrink-0 rounded bg-success-subtle" style="width: 100px; height: 60px;">

                                                <!-- Display the image if it exists, otherwise show a placeholder -->
                                                    <img
                                                    src="${currency.flag ? '/files/images/country/' + currency.flag : 'https://placehold.co/100x60'}"
                                                    alt="Country Image"
                                                    class="img-fluid rounded"
                                                    style="width: 100px; height: 60px;">
                                                </div>
                                            <h6 class="text-end ">
                                                <a href="${editRoute.replace(':id', currency.id)}" class="text-decoration-none text-black">${currency.name}</a>
                                            </h6>
                                        </div>
                                        <!-- <div class="card-footer text-end mt-2">
                                            <a href="${editRoute.replace(':id', currency.id)}" class="btn btn-primary btn-sm" title="Edit">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <button class="btn btn-danger btn-sm delete-currency" data-id="${currency.id}" title="Delete">
            ...
            $(document).on('click', '.btnRefreshCard', function() {
                $('#tableSearch').val('');
                currentPage = 1;
                loadCountriesGridView(false, null, 1);
            });
            ...
            $(document).on('click', '#confirmDelete', function() {
                ...
                const deleteCountryUrl = "{{ route('countries.destroy', ':id') }}".replace(':id', id);
                $.ajax({
                    ...
                    error: function(xhr) {
                        $('#deleteCardModal').modal('hide'); // Hide the modal
                        changeButton("#confirmDelete", false);
                        showToast('error', 'Failed to delete the country. Please try again.');
            ...
            $('.card-container').on('scroll', function() {
                ...
                if (container.scrollTop() + container.innerHeight() >= container[0].scrollHeight - 50) {
                    ...
                    if (!isLoading && dataSize >= 1) {
                        ...
                        loadCountriesGridView(true); // Load more data
                    }
                }
            });
            // Initial load
            ...
            searchInput.on('input', function() {
                ...
                debounceTimer = setTimeout(function() {
                    loadCountriesGridView(false, searchQuery,
            ...
            function loadCountriesDataTable() {
                ...
                let table = $('#currencies-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('countries.getCountries') }}",
                    dataSrc: '',
                    responsive: true,
                    ...
                    columns: [
                        ...
                        {
                            data: 'flag', // This field should contain the filename of the flag (e.g., 'us.png')
                            orderable: false,
                            searchable: false,
                            render: function(data, type, row) {
                                // Use Laravel's asset helper for images in the public directory
                                let baseUrl =
                                    "{{ asset('files/images/country') }}"; // Laravel will generate the correct path
                                let placeholder =
                                    "{{ asset('files/images/country/placeholder.png') }}"; // Placeholder image path

                                // Check if data exists, otherwise use the placeholder
                                let flagImage = data ? `${baseUrl}/${data}` : placeholder;

                                return `<img src="${flagImage}" alt="Flag" style="width: 40px; height: 25px; object-fit: cover; border: 1px solid #ddd; border-radius: 2px;">`;
                            },
                            width: "5%"
                        },
                        {
...

                        {
                            ...
                            name: 'is_active',
                            render: function(data, type, row) {
                                // Check if the `is_active` value is true
                                if (data) {
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                const viewRoute =
                                    "{{ route('countries.view', ['country' => ':id']) }}";
